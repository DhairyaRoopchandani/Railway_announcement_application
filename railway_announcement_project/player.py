...
class Player(tk.Frame):
	...
	def create_frames(self):
		...
		self.tracklist = tk.LabelFrame(self, text=f'List - {str(len(self.playlist))}',
							font=("times new roman",15,"bold"),
							bg="grey",fg="white",bd=5,relief=tk.GROOVE)
	...
	def track_widgets(self):
		...
		self.songtrack['text'] = 'Announcement MP3 Player'
		self.songtrack.config(width=30, height=1)
		self.songtrack.grid(row=1,column=0,padx=10)

	def control_widgets(self):
	    #clicking on load list button,all the announcements will be displayed in the playlist
		self.loadSongs = tk.Button(self.controls, bg='green', fg='white', font=10)
		self.loadSongs['text'] = 'Load list'
	...
	def retrieve_songs(self):
		self.songlist = []
		os.system('main.py')
		directory = 'C:\\Users\\Dhairya\\OneDrive\\Desktop\\announcements'
...
root.wm_title('Announcements')

img = PhotoImage(file='images/train.gif')
